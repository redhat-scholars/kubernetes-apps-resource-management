...
These parameters will determine the the garbage collector (GC) algorithm and the performance of the application.
...
`-Xlog:gc=info`:: Shows information about GC (i.e which algortihm is used)
...
<2> Using GC serial as tehre is only one CPU available
<3> Uses CGroups v1
...
At this point, we can do two things to adjust the limits: 
...
[source,bash]
----
export ROUTE_URL=http://$(kubectl get route anpther-greeting-app -o jsonpath='{.spec.host}')
